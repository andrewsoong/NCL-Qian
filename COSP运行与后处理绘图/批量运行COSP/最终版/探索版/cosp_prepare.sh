...
do
	filename=(`ls /home/yongluo/WORK/cosp/wrf_file/$year/$month/wrfout*`)
	num=${#filename[@]}
	for ((i=0;i<=num-1;i++))
	do
		...
		sed -i "123s/^.*.$/	fout    = addfile(\"${fileout}.nc\",\"c\")/" input_prepare.ncl
		ncl input_prepare.ncl
		mv ${fileout}.nc /home/yongluo/WORK/cosp/forcosp/$year/$month/
