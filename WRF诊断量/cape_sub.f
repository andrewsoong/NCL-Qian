C NCLFORTSTART
	subroutine CAPE_2D(L,M,N,P3d,T3d,R3d,PAR3d,PAP3d,NAR3d,NAP3d,
     +             CAPER3d,CAPEP3d,DCAPE3d)
	real P3d(L,M,N),T3d(L,M,N),R3d(L,M,N),PAR3d(L,M,N),
     +       PAP3d(L,M,N),NAR3d(L,M,N),NAP3d(L,M,N),
     +       CAPER3d(L,M,N),CAPEP3d(L,M,N),DCAPE3d(L,M,N)
C NCLEND
        PARAMETER(NA=60)
	...
	CALL CAPE(N,P,T,R,PAR,PAP,NAR,NAP,CAPER,CAPEP,DCAPE)
	do k = 1, N
	PAR3d(i,j,k) = PAR(k)
	PAP3d(i,j,k) = PAP(k)
        NAR3d(i,j,k) = NAR(k)
	NAP3d(i,j,k) = NAP(k)
	CAPER3d(i,j,k) = CAPER(k)
        CAPEP3d(i,j,k) = CAPEP(k)
        DCAPE3d(i,j,k) = DCAPE(k)
	end do
	end do
	end do
	return
...
C NCLFORTSTART
	subroutine CAPE(N,P,T,R,PAR,PAP,NAR,NAP,CAPER,CAPEP,DCAPE)
C
...
C
        REAL T(N), P(N), R(N),CAPER(N), CAPEP(N), DCAPE(N), PAR(N)
        REAL PAP(N), NAR(N), NAP(N)
C NCLEND
	PARAMETER (NA=60)
